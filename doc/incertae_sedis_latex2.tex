...
\title{\texttt{\emph{Incertae sedis}}\texttt{ taxa in supertree construction}}
\maketitle
...
We describe an extension to our supertree method so that we can resolve
...
Formalism. We consider a ranked collection $\phyloinputs=\left\{ T_{1},\ldots,T_{n}\right\} $
...
Furthermore, the set of labels $\mathcal{L}(i)$ on input tree $T_{i}$
need not include all leaf labels, and in practice always contains
only a relatively small part of $\mathcal{L}$.

...
The taxonomy tree is considered by ranked below any of the input trees.
Each node $i$ in the taxonomy tree is associated a taxon name $n(i)$
in the set of taxon names $\mathcal{N}$, and each taxon name is corresponds
...
total number of taxa. Furthermore, taxa in input trees need not correspond
to leaf taxa on the taxonomy tree, and multiple tips may be labeled
...
\subsubsection{Partially reduced exclude sets for \emph{incertae sedis} taxa}
...
then we have 
\begin{align}
\mathcal{E}(n) & =\mathcal{E}(parent(n))\cup\left[\mathcal{I}(m)\big|m\in siblings(n),m\text{ not \emph{incertae} \emph{sedis}}\right],\label{eq:exclude-set-formula-1}
\end{align}
...
\subsubsection{Extended formulation}

If an \emph{incertae sedis} taxon $A$ is placed within a sibling
clade, then it may gain a new \emph{incertae sedis} sibling $B$.
We could then possibly place $B$ within $A$. However, the above
definition does not allow this possibility. We therefore adjust the
above definition to exclude only descendants of sibling taxon only
the path to the descendant does not pass through a node labeled as
\emph{incertae sedis}. Such descendants of a sibling $m$ are given
by:
\begin{align*}
...
\end{cases}
\end{align*}
We then modify the formula above to exclude $\mathcal{I}^{\prime}(m)$
instead of $\mathcal{I}(m)$ for each sibling $m$:
\begin{align}
\mathcal{E}^{\prime}(n) & =\mathcal{E}^{\prime}(parent(n))\cup\left[\mathcal{I}^{\prime}(m)\big|m\in siblings(n),m\text{ not \emph{incertae} \emph{sedis}}\right],\label{eq:exclude-set-formula-2}
\end{align}

This less stringent formula is the one we will apply throughout this
paper.

\subsubsection{Comparison to operational definition}

An definition in terms of moving node $A$ to position $x$ could
cause trouble when moving multiple nodes, since the order of the movement
would matter.

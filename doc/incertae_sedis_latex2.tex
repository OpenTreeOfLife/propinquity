...
been estimated from DNA sequence data, we additionally seek to include
taxonomic information. We thus require that one of the input trees
is the taxonomy tree. The taxonomy has a number of features not found
in the input trees, such as having labels for internal nodes, containing
monotypic nodes, and containing \emph{incertae sedis} nodes. We refer
to labels of taxonomy nodes as ``taxa'', and require that labels
and taxonomy nodes have a one-to-one correspondence. We additionally
require that the taxonomy be comprehensive and contain all labels.

One of the problems that arises when attempting to include taxonomic
information is the problem of \emph{incertae sedis }taxa. \emph{Incertae
sedis} taxa are taxa that have unknown taxonomic placement. The common
interpretation of such taxa is that they may not be place further
root-ward, but may be placed within their sibling taxa. This frequently
occurs when a specimen is identified to a taxonomic level such as
the family level but not further. It also occurs when it is unclear
whether a genus forms its own family, or is part of an existing family.
(Mark rewrite?)
...
We extend this framework by adding a set $\mathcal{U}\subseteq\mathcal{N}$
...
We define rooted splits by noting that each edge of a tree divides
the tip taxa $\mathcal{L}$ into two groups: the include set $\mathcal{I}(e)$
which does not contain the root, and the exclude set $\mathcal{E}(e)$
...
of all the ancestors of $n$, unless those children are \emph{incertae
sedis}. 
...
to a sibling $B$. If $B$ is also \emph{incertae sedis}, then it
should be possible to place $B$ within $A$. As a result, $A$ should
not exclude descendants of $B$ than can only be reached from $A$
by passing through a node marked \emph{incertae sedis. }
...
one name to apply to multiple nodes. For example, in Figure \ref{fig:One-name-can},
the name $A$ can apply to multiple nodes. Here the name $A$ corresponds
to the split $A1\,A2\,|\bullet B\,C$, leaving out $D$ since it is
\emph{incertae sedis}. The two nodes $x$ and $y$ imply the splits
$A1\,A2\,|\bullet B\,C\,D$ and $A1\,A2\,D|\bullet B\,C$ respectively,
and both of these splits imply the split $A1\,A2\,|\bullet B\,C$,
so the name $A$ can apply to both $x$ and \footnote{Define ``implies'', as in the propinquity paper: $A_{1}|\bullet A_{2}\implies B_{1}|\bullet B_{2}$
...
we find the most tip-ward node where the name can apply and attach
the name to this node. This corresponds to a conservative choice about

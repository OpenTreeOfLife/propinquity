...
equation (\ref{eq:exsetformonea}) is a unique an uncontroversial
...
\ref{fig:Placing-one-incertae-MRP}b could represent this taxonomy 
...
The matrix shown in Figure \ref{fig:Placing-one-incertae-MRP}b allows
...
non-monophyletic, the matrix from Figure \ref{fig:Placing-one-incertae-MRP}b
allows $W$ to be placed within $V$.  In general, this approach would
allow incertae sedis children of any taxon $\Gamma_1$ to be placed
...
\ref{fig:Placing-one-incertae-MRP}b comprise a unique, best
...
Figure \ref{fig:Placing-one-incertae-MRP}c.  Running the supertree
...
While the topology for the final tree would indeed be the tree shown in 
    ...
    representation of $\taxonomy$ in Figure \ref{fig:Placing-one-incertae-MRP}b
...
\caption{\label{fig:Placing-one-incertae}
  ...
  exclude sets that is shown in \ref{fig:Placing-one-incertae-MRP}b,
...
The representations only vary in the scoring of leaf $W$ for character $V$. Panel (c) shows the information encoded by the tree in \ref{fig:Placing-one-incertae}b. }
\end{figure}

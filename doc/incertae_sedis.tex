...
\mthcomment{Need to verify that the namer records this situation as synonyms!}
...
Thus $T1$ is placed within $R$.
We note that the MRCA of $R$ and $T1$ in
the taxonomy is the root node, and the path to the MRCA from $R$ is
...
%We think that they constitute one of the main contributions of the paper.
In each case, the synthesis trees in this section are constructed using the
...
With the naive semantics, intrusion of an \emph{incertae sedis} taxon into a sibling is
...
We see from Fig. \ref{fig:Handling-incertae-sedis}d that use of the unrestricted intrusion
...
\subsection{Conflict with \emph{incertae sedis} taxa}
...
In Figure \ref{fig:Broken-incertae-sedis}, tree $T_{1}$ places $T1$
next to $R1$, while $T_{2}$ places $T2$ next to $R2$.
Since different
members of $C$ are placed, neither placement for $T$ is rejected.
Instead the taxon $T$ is broken, the name $T$ disappears, and $T3$
floats to the MRCA of the placed members of $T$.
Furthermore, since taxon $T$ is a broken
...
Therefore, the names $R$ and $RS$ are not
lost, since the taxa placed within them are \emph{incertae sedis}
names.

The situation here would be different if the monophyly of $T$ was
supported by an high-ranked input phylogeny.
In that case, instead of
breaking $T$, we would choose the placement in the highest-ranked
tree.
...
\bdrcomment{Ben: discuss figure \ref{fig:interdigitation}.}
...
\bdrcomment{Ben: Write discussion of Figure \ref{fig:Placing-one-incertae}}
...
\begin{eqnarray*} 
\mathcal{E}(n) & =\mathcal{E}(parent(n))\cup\left[\mathcal{I}(m,n)\big|m\in siblings(n)\right].\label{eq:exsetformoneb}
\end{eqnarray*}
...
\bdrcomment{We get Regulus back, bu we do not end up placing it in Sylviidae
because Sylviidae is broken. It conflicts with about 20 different trees.}

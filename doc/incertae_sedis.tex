...
\emph{incertae sedis} are shaded grey. \mthcomment{We should probably remove $C3$ and replace $B1$ and $B2$ with $B$
to simplify this. We could use triangle to indicate that $D$ and $B$ could be leaves or clades.}
}
...
In this paper we've described a method for handling \emph{incertae sedis} taxa
when constructing taxonomic supertrees.  Our method incorporates the uncertain
placement information from such taxa when constructing the supertree.  It also
...
However, our method leaves a number of problems for future work.  One of these
problems is that we consider taxa with an \emph{incertae sedis} label to 
about which siblings in attach within.  However, in practice,
the taxonomist may only consider it plausible for the taxon to intrude only within specific
siblings.  Our method also assumes that an \emph{incertae sedis} label on a taxon

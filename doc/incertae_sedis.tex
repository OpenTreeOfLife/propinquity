...
number of taxa into groups that are marked as ``classified''. or
...
First, this approach results in a less-resolved tree because taxonomic
vagueness (in the form of the \emph{incertae sedis} annotation) is interpreted as
an unwarranted statement that the \emph{incertae sedis} taxa are disallowed from
being members of their sibling taxa.  When an incertae sedis taxon is placed within
a sibling taxon by input phylogenies, this is taken as a rejection of the monophyly
of the sibling taxon, which is then removed from the synthesis tree.  For
example, in the synthesis tree in Fig. \ref{fig:Handling-incertae-sedis}c, the
placement of taxon $C$ in group $AB$ is interpreted as a rejection of the
monophyly of $AB$. The edge leading to $AB$ is therefore not incorporated into
the synthesis tree.  In contrast, the synthesis tree in Fig.
\ref{fig:Handling-incertae-sedis}d that does not reject the monophyly of $AB$
retains the grouping of $A$ and $B$.

Second, this approach results in the removal of clade names for all clades that
have an \emph{incertae sedis} taxon place within them.  For example, in Fig.
\ref{fig:Handling-incertae-sedis}c the names for the clades $A$ and $AB$ are lost.

Third, this approach cannot use information from phylogenies to resolve the
...
allow us to place $C$ within $A$.  In contrast, the synthesis tree in Fig.
\ref{fig:Handling-incertae-sedis}d that takes account of \emph{incertae sedis}
annotations and retains the label for $A$.  This allows us to infer that
\emph{incertae sedis} taxon $C$ has been placed within $A$, resolving
the uncertainty.
...
% Question: why not use rank information?  Answer: well, we could, but lets start simple.

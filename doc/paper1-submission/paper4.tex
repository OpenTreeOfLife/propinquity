...
the sum of displayed groups' weighted scores criterion. This is accomplished
...
\begin{tabular}{lcccccc}
 & \supportedby{} & \partialpathof{} & \terminal{} & \conflictswith{} & \resolvedby{} & \resolves{}\tabularnewline
\hline 
\otl{} & 125,384 & 0 & 2,424,255 & 1998 & 4 & 3676\tabularnewline
\hline 
...
input phylogeny splits considered thus 40996.

The number of displayed input splits (\supportedby{}
+ \partialpathof{}) increased from 35,518 (for \otl{}) to 39,639 (for \otlprop{});
an 11.2\% increase. When examining which splits are displayed, we find
that the \otlprop{} tree displays 4920 input splits that
...
splits that conflict with the summary (conflicts\_with) dropped from
2760 to 1357, a decrease of 1357, or 49.2\%.  In accordance with the
...
incorporated (resolved\_by) dropped from 2718 to 0.  We also find that
...
project to build its summary tree.  Our new method represented 11\%
more input phylogeny splits with 49\% less conflict compared to the
Open Tree of Life version 4 summary tree when applied to the same inputs.
Unless the previous method \citep{HinchliffEtAl2015}, our new method
is guaranteed to incorporate input splits that do not conflict with
the summary tree. The method is implemented in the Open Source
software package propinquity.  A modified version of the treemachine
software which built the summary tree described in the
\citet{HinchliffEtAl2015} paper is used by the project to serve the
tree produced by propinquity via Open Tree of Life APIs. 

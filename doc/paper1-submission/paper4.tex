...
summarizes a collection of input phylogenies and an input taxonomy.
We introduce formal requirements that such a supertree must satisfy in
order to transparently and justifiably represent the input trees.   In
addition to producing a tree, our method writes an annotations file
that reports which grouping in the input trees support each of the
groups in the supertree.

We compare the performance of this new method to a previous supertree
method on the inputs to the Open Tree of Life version 4, and find that
the number of displayed input splits increases from 35,518 to 39,639 and the
number of conflicting input splits decreases from 2760 to 1357.  The
new supertree method also improves on the previous supertree
construction method because it produces no unsupported branches, and
it avoids unnecessary polytomies. 

...
\caption{Exemplified input trees (a-b) and pruned taxonomy tree (c) from Fig. \ref{fig:cleaned-phylo-example}.
Taxon E in the first input tree is exemplified by E1 in (a). Pruned taxa are E2, F2, and D. The
taxa E and F are retained as monotypic taxa in the pruned taxonomy. The red
...
tool in otcetera; see appendix B for
...
\caption{Subproblems (a) \emph{ABCD} and (b) \emph{root} generated from the exemplified trees shown in Fig. \ref{fig:exemplify-example}.
...
\caption{Solutions to (a) subproblem \emph{ABCD} and (b) subproblem \emph{root} depicted in Fig. \ref{fig:subproblems-example}.}
...
tree $\protect\summaryTree$ in panel (b) to edges in the input tree $T_{1}$ named ``tree1'' in panel
(a). Only edges of $\protect\summaryTree$ that are present in the
induced tree $\protect\summaryTree(1)$ in panel (c) are represented by JSON annotations
in panel (d). Taxon names are here suppressed in favor of OTT IDs, and edges

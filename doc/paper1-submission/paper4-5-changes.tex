...
%DIF DEL paper4.tex            Thu Oct 20 12:15:37 2016
%DIF ADD paper5-adjusted.tex   Mon Jan 30 17:28:48 2017
...
\begin{abstract}
%DIF 84c84
%DIF < We present a new supertree method that enables rapid estimation of a
%DIF -------
We present a new supertree method that enables rapid construction of a %DIF > 
%DIF -------
...
%DIF 96c96
%DIF < supertree construction method by assessing their performance on input
%DIF -------
method by assessing their performance on input %DIF > 
%DIF -------
...
\DIFdelbegin \DIFdel{Let $\summaryTree$ denote a supertree, and $T_{i}$ denote the $i$th
...
the induced tree. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We can consider whether or not a node in an input tree is displayed
by $\summaryTree$.
For any such node }\DIFdelend \DIFaddbegin \DIFadd{Cutting any edge }\DIFaddend $j$ \DIFdelbegin \DIFdel{there is a set of taxa
that are mapped to the tips that descend from the node. This set of taxa is the }\DIFdelend \DIFaddbegin \DIFadd{of a rooted tree induces a bi-partition
$S(j)$ of the leaf taxa into two connected groups $S_{1}(j)$ and
$S_{2}(j)$. Such a bi-partition is called a rooted split and written
$S(j)=S_{1}(j)|\bullet S_{2}(j)$. Here $S_{1}(j)$ is called the }\DIFaddend ``cluster'' of \DIFdelbegin \DIFdel{taxa corresponding to
node }\DIFdelend $j$\DIFdelbegin \DIFdel{; it can
be denoted $\mathcal{L}(i,j)$. It will also be referred to as the ``include set'' of the node. The ``exclude set'' of }\DIFdelend \DIFaddbegin \DIFadd{, and contains tip taxa on the side of edge $j$ that does not contain the
root. $S_{2}(j)$ contains the tip taxa on the side of edge }\DIFaddend $j$ \DIFdelbegin \DIFdel{in $T_{i}$
is the set of taxa in $\mathcal{L}(i)$ but not in $\mathcal{L}(i,j)$. If }\DIFdelend \DIFaddbegin \DIFadd{that
contains the root. We refer to $S(j)$ as a }\emph{\DIFadd{rooted}} \DIFadd{split because
the right side of the split implicitly contains the root node, as indicated
by }\DIFaddend the \DIFdelbegin \DIFdel{cluster of taxa for any supertree node in $\summaryTree(i)$
is identical to $\mathcal{L}(i,j)$, then }\DIFdelend \DIFaddbegin \DIFadd{$\bullet$.
}

\DIFadd{For any two rooted splits $A=A_{1}|\bullet A_{2}$ and $B=B_{1}|\bullet B_{2}$,
we say that $A$ }\emph{\DIFadd{displays}} \DIFadd{$B$ if $B_{1}\subseteq A_{1}$ and
$B_{2}\subseteq A_{2}$. We also say that $A$ and $B$ }\emph{\DIFadd{conflict}}
\DIFadd{if none of $A_{1}\cap B_{1}$, $A_{1}\cap B_{2}$, and $B_{2}\cap A_{1}$
are empty.
...
sides implicitly contain the root. If $A$ and $B$ do not conflict, then we say that they are }\emph{\DIFadd{compatible}}\footnote{\DIFadd{It is possible to construct a rooted tree that displays both $A$
and $B$ if and only if $A$ and $B$ are compatible}}\DIFadd{. If $A$ displays $B$, then $A$ and $B$ must be compatible, since
$A_{1}\cap B_{2}$ and $B_{1}\cap A_{2}$ must be empty. We adopt
the shorthand of referring to an edge $k$ when we mean the split
$S(k)$. So, when }\DIFaddend we say that \DIFdelbegin \DIFdel{$\summaryTree$
displays node }\DIFdelend \DIFaddbegin \DIFadd{edge }\DIFaddend $j$ \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{conflicts with edge $k$,
we mean that $S(j)$ conflicts with $S(k)$. 
}

\DIFadd{Let $\summaryTree$ denote a supertree, and }\DIFaddend $T_{i}$ \DIFaddbegin \DIFadd{denote the $i$th
input tree}\DIFaddend . We say that \DIFdelbegin \DIFdel{the summary tree }\emph{\DIFdel{displays}}
%DIFAUXCMD
\DIFdel{edge $j$ if the summary tree displays the child node of }\DIFdelend \DIFaddbegin \DIFadd{$\summaryTree$ displays an }\DIFaddend edge $j$ \DIFdelbegin \DIFdel{.
Operationally, we can find the most recent common ancestor (MRCA)
node of $\mathcal{L}(i,j)$ in }\DIFdelend \DIFaddbegin \DIFadd{of $T_{i}$
if any edge of }\DIFaddend $\summaryTree$ \DIFdelbegin \DIFdel{; the summary tree }\DIFdelend displays $j$\DIFdelbegin \DIFdel{if and only if that MRCA node is not an ancestor of any member
of the exclude set of }\DIFdelend \DIFaddbegin \DIFadd{. We say that $\summaryTree$
conflicts with a edge $j$ of $T_{i}$ if any edge of $\summaryTree$
conflicts with $j$; otherwise $\summaryTree$ is compatible with
}\DIFaddend $j$. We say that \DIFdelbegin \DIFdel{node }\DIFdelend \DIFaddbegin \DIFadd{edge }\DIFaddend $k$ of \DIFdelbegin \DIFdel{the summary tree
}\DIFdelend \DIFaddbegin \DIFadd{$\summaryTree$ }\DIFaddend is \emph{supported
by} \DIFdelbegin \DIFdel{node }\DIFdelend \DIFaddbegin \DIFadd{edge }\DIFaddend $j$ \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend $T_{i}$ if \DIFdelbegin \DIFdel{the summary tree displays node }\DIFdelend \DIFaddbegin \DIFadd{$\summaryTree$ displays }\DIFaddend $j$, but \DIFaddbegin \DIFadd{$\summaryTree$
would not display $j$ }\DIFaddend if we contracted edge \DIFdelbegin \DIFdel{that separates }\DIFdelend $k$\DIFdelbegin \DIFdel{from its parent then the modified summary tree would no longer display node }\DIFdelend \DIFaddbegin \DIFadd{. We also adopt the
shorthand of referring to edges by their tipward endpoint. By combining
this with the above shorthand, we could say that a node }\DIFaddend $j$ \DIFaddbegin \DIFadd{conflicts
or does not conflict with another node $k$.
}

\DIFadd{The set of taxa that are mapped to the tips of the tree $T_{i}$ is
...
is an ancestor of some but not all of these leaves}\DIFaddend . 
...
shown in \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:toyambig}; panels \DIFdelbegin \DIFdel{(\ref{fig:toyambig}a
)
and (\ref{fig:toyambig}b ) }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:toyambig}a
and \ref{fig:toyambig}b }\DIFaddend show two input trees. Because taxa A and
...
in \DIFdelbegin \DIFdel{(\ref{fig:toyambig}c ) or (\ref{fig:toyambig}d ) }\DIFdelend \DIFaddbegin \DIFadd{Fig. \ref{fig:toyambig}c or Fig. \ref{fig:toyambig}d }\DIFaddend would be characterized
...
In \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:toyambig}, taxon E floats to different positions
in \DIFdelbegin \DIFdel{(\ref{fig:toyambig}c ) and (\ref{fig:toyambig}d)}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:toyambig}c and \ref{fig:toyambig}d}\DIFaddend , whereas taxon
A does not. 

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Input tree 1]{\includegraphics[width=0.45\textwidth]{Figures__toy_ambig__abcd}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Input tree 2]{\includegraphics[width=0.45\textwidth]{Figures__toy_ambig__bcde}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < \subfloat[Summary tree]{\includegraphics[width=0.45\textwidth]{Figures__toy_ambig__soln1}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Summary tree]{\includegraphics[width=0.45\textwidth]{Figures__toy_ambig__soln2}
%DIFDELCMD < 
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Input tree 1]{\includegraphics[scale=0.57]{Figures__toy_pairwise_compat__abc}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Input tree 2]{\includegraphics[scale=0.57]{Figures__toy_pairwise_compat__acd}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Input tree 3]{\includegraphics[scale=0.57]{Figures__toy_pairwise_compat__bcd}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < \subfloat[Summary tree]{\includegraphics[scale=0.57]{Figures__toy_pairwise_compat__s12}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Summary tree]{\includegraphics[scale=0.57]{Figures__toy_pairwise_compat__s13}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Summary tree]{\includegraphics[scale=0.57]{Figures__toy_pairwise_compat__s23}
%DIFDELCMD < 
...
\DIFaddbegin \subsubsection{\DIFadd{Non-goal: phylogeny estimation}}
\DIFadd{Many researchers construct supertrees for the purpose of inferring the true tree from the input trees \mbox{%DIFAUXCMD
\citep{BinindaEmonds2007b,Davis2014}}%DIFAUXCMD
.  Such approaches treat input trees as data or as surrogates for data matrices \mbox{%DIFAUXCMD
\citep{gatesy2004critique}}%DIFAUXCMD
, and the supertree is seen as more accurate than the individual input trees because more input trees (and presumably more data) stand behind it.  For such methods, conflict resolution is a primary aim of the method.
}

\DIFadd{Our supertree method operates in a different paradigm.  Phylogeny estimation is an explicit non-goal of our supertree algorithm.  We do not claim that the supertree is more accurate than the input trees it summarizes, and so conflict resolution is not a primary aim of the method. Most of our input trees have non-overlapping taxon sets, and so conflict resolution is not the primary problem.  Instead, we seek to merge and aggregate phylogenetic information on the largest possible scale.  Therefore, the primary aim of our supertree method is to }\emph{\DIFadd{summarize and represent}} \DIFadd{the input trees, even when they conflict with each other.  The annotations file constructed by our pipeline is essential for this task because it includes information about conflict with our supertree as well as support for it.
}

\DIFadd{Our method must resolve conflicts in order to construct a single supertree.  However, the rank information used to resolve conflicts is an input to the method, not an output from the method.  We thus perform curation-based conflict resolution, not inference-based conflict resolution.
}

\DIFaddend \subsubsection{Summary of goals}
...
from lower ranked trees.  \DIFaddbegin \DIFadd{These large weights help to achieve our goal of allowing curators to fix problems in the tree.  While it is possible to view our pipeline as an optimization algorithm by assigning weights to the input trees in this way, we are not trying to weight the evidence in the input trees, and we do not actually ever calculate these weights in our pipeline.
...
\subsubsection{\DIFadd{Comparison to other supertree methods}}

\DIFadd{Matrix Representation Parsimony (MRP) is one of the
most widely used supertree approaches \mbox{%DIFAUXCMD
\citep{baum1992combining,BinindaEmonds2007b,Davis2014}}%DIFAUXCMD
.
...
violates our Goal \#1 \mbox{%DIFAUXCMD
\citep{gatesy2004critique}}%DIFAUXCMD
.
}

\DIFadd{Another commonly used method is MinCut \mbox{%DIFAUXCMD
\citep{semple2000supertree} }%DIFAUXCMD
or
Modified MinCut \mbox{%DIFAUXCMD
\citep{page2002modified}}%DIFAUXCMD
.  Like our approach, MinCut
is based on the BUILD algorithm \mbox{%DIFAUXCMD
\citep{AhoSSU1981}}%DIFAUXCMD
.  However, MinCut deals
...
usability and reliability. The first steps of the pipeline (see \DIFdelbegin \DIFdel{Figure
}\DIFdelend \DIFaddbegin \DIFadd{Fig.
}\DIFaddend \ref{fig:pipeline}) collect a list of trees to include (in the \texttt{phylo\_input}
...
\begin{figure}
\begin{centering}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=0.70\paperheight]{pipeline-tools.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.70\paperheight]{Fig3.pdf}
...
been sampled in a phylogenetic input. \DIFdelbegin \DIFdel{Figure \ref{fig:exemplify-example}
shows an exampleof how }\DIFdelend \DIFaddbegin \DIFadd{For example, }\DIFaddend the trees in \DIFdelbegin \DIFdel{figure \ref{fig:cleaned-phylo-example} would be exemplified }\DIFdelend \DIFaddbegin \DIFadd{Fig.
\ref{fig:cleaned-phylo-example} would exemplified as shown in Fig.
\ref{fig:exemplify-example}}\DIFaddend .
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Tree 1]{\includegraphics[scale=0.4]{Figures__cleaned_phylo__ex_2@tree1\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Tree 2]{\includegraphics[scale=0.4]{Figures__cleaned_phylo__ex_2@tree2\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Taxonomy Tree]{\includegraphics[scale=0.4]{Figures__cleaned_ott__cleaned_ott\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{Fig4}
\DIFaddendFL 

\caption{Input trees \DIFaddbeginFL \DIFaddFL{(a-b) }\DIFaddendFL and taxonomy tree \DIFaddbeginFL \DIFaddFL{(c).}\DIFaddendFL }
\label{fig:cleaned-phylo-example}
\end{figure}

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Tree 1]{\includegraphics[scale=0.4]{Figures__exemplified_phylo__ex_2@tree1\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Tree 2]{\includegraphics[scale=0.4]{Figures__exemplified_phylo__ex_2@tree2\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Pruned taxonomy tree $\protect\prunedTaxonomy$]{\includegraphics[scale=0.4]{Figures__exemplified_phylo_taxonomy_uncontested\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{Fig5}
\DIFaddendFL 

\caption{Exemplified input trees \DIFaddbeginFL \DIFaddFL{(a-b) }\DIFaddendFL and \DIFaddbeginFL \DIFaddFL{pruned }\DIFaddendFL taxonomy tree \DIFaddbeginFL \DIFaddFL{(c) }\DIFaddendFL from \DIFdelbeginFL \DIFdelFL{figure }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fig. }\DIFaddendFL \ref{fig:cleaned-phylo-example}.
\DIFaddbeginFL \DIFaddFL{Taxon }\DIFaddendFL E in \DIFdelbeginFL \DIFdelFL{tree1 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the first input tree }\DIFaddendFL is exemplified by E1 \DIFaddbeginFL \DIFaddFL{in (a)}\DIFaddendFL . Pruned taxa are E2, F2, and D. The
taxa E and F are retained as monotypic taxa in the pruned taxonomy\DIFdelbeginFL \DIFdelFL{$\protect\prunedTaxonomy$, but are not shown in panel c}\DIFdelendFL . The red
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Subproblem \emph{ABCD}]{\includegraphics[width=0.3\textwidth]{Figures__subproblems__14__1\lyxdot tre}%
%DIFDELCMD <     \hfill{}\includegraphics[width=0.3\textwidth]{Figures__subproblems__14__2\lyxdot tre}%
%DIFDELCMD <     \hfill{}\includegraphics[width=0.3\textwidth]{Figures__subproblems__14__3\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD <   \subfloat[Subproblem \emph{root}]{\includegraphics[width=0.3\textwidth]{Figures__subproblems__17__1\lyxdot tre}%
%DIFDELCMD <     \hfill{}\includegraphics[width=0.3\textwidth]{Figures__subproblems__17__2\lyxdot tre}%
%DIFDELCMD <     \hfill{}\includegraphics[width=0.3\textwidth]{Figures__subproblems__17__3\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{Fig6}
\DIFaddendFL 

\caption{Subproblems \DIFaddbeginFL \DIFaddFL{(a) }\emph{\DIFaddFL{ABCD}} \DIFaddFL{and (b) }\emph{\DIFaddFL{root}} \DIFaddendFL generated from the exemplified trees shown in \DIFdelbeginFL \DIFdelFL{figure }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fig. }\DIFaddendFL \ref{fig:exemplify-example}.
A trivial \DIFdelbeginFL \DIFdelFL{statemen }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{statement }\DIFaddendFL from the first tree that a taxon labelled $ABCD$
...
\end{figure}
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[]{\includegraphics[scale=0.57]{Figures__uncontested_worsens_score__ab1c}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[]{\includegraphics[scale=0.57]{Figures__uncontested_worsens_score__acb2}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[]{\includegraphics[scale=0.57]{Figures__uncontested_worsens_score__tax}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < \hfill{}\subfloat[]{\includegraphics[scale=0.57]{Figures__uncontested_worsens_score__opt}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[]{\includegraphics[scale=0.57]{Figures__uncontested_worsens_score__returned}
%DIFDELCMD < 
...
the current set of splits (\DIFdelbegin \DIFdel{Alg }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{alg:ConsistentSplitsFromRankedList}).
...
%DIFDELCMD < \Require %%%
\DIFdel{An ordered list of $M$ splits, $\mathcal{R} = [R_1, R_2, R_3, \ldots, R3, \ldots, R_M]$
}%DIFDELCMD < \State %%%
\DIFdel{$\mathcal{C} = [R_1]$
}%DIFDELCMD < \For{each split $i$ in $[2, 3 \ldots M]$}
%DIFDELCMD <    \State %%%
\DIFdel{$\mathcal{T} \leftarrow \mathcal{C} + R_i$ }%DIFDELCMD < \Comment{where `+' means concatenating 2 lists}
%DIFDELCMD <    \If{\textsc{BUILD}$(\mathcal{T})$ does not return null}
%DIFDELCMD <       \State %%%
\DIFdel{$\mathcal{C} \leftarrow \mathcal{T}$
   }%DIFDELCMD < \EndIf
%DIFDELCMD < \EndFor{}
...
\DIFaddendFL \caption{\label{alg:ConsistentSplitsFromRankedList}\DIFdelbeginFL \DIFdelFL{ConsistentSplitsFromRankedList}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Algorithm }\texttt{\DIFaddFL{ConsistentSplitsFromRankedList}}\DIFaddendFL }
\DIFdelbeginFL %DIFDELCMD < \end{algorithm}
%DIFDELCMD < %%%
...
at their common ancestor. \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:subproblem-solutions.}
shows the solutions that would be obtained by applying our modified
version of the BUILD algorithm to the subproblems shown in \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:subproblems-example}.

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \hfill{}\subfloat[Solution to subproblem \emph{ABCD}]{\includegraphics[scale=0.55]{Figures__subproblem_solutions__ott14\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Solution to subproblem \emph{root}]{\includegraphics[scale=0.55]{Figures__subproblem_solutions__ott17\lyxdot tre}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{Fig9}  
\DIFaddendFL 

\caption{Solutions to \DIFdelbeginFL \DIFdelFL{the subproblems }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(a) subproblem }\emph{\DIFaddFL{ABCD}} \DIFaddFL{and (b) subproblem }\emph{\DIFaddFL{root}} \DIFaddendFL depicted in \DIFdelbeginFL \DIFdelFL{figure }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fig. }\DIFaddendFL \ref{fig:subproblems-example}\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }
...
by propinquity)\DIFaddbegin \DIFadd{; see Fig. \ref{fig:grafted-solution}}\DIFaddend . Recall that each non-root uncontested taxon used
...
\begin{figure}
\begin{centering}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.5]{Figures__grafted_solution__grafted_solution_ottnames\lyxdot tre}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.5]{Fig10.pdf}
\DIFaddendFL \par\end{centering}
\caption{Grafted solution \DIFaddbeginFL \DIFaddFL{produced from the subproblems from
Fig}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{The nodes E }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{fig:subproblem-solutions.} }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{F }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{which is the 
backbone onto which taxa that }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{monotypic here, but }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{not included in any phylogeny
}\DIFaddendFL will \DIFdelbeginFL \DIFdelFL{end
up being polytypic after unpruning is performed}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{be placed}\DIFaddendFL .}
\DIFaddbeginFL \label{fig:grafted-solution}
...
the taxon in the grafted solution tree.  \DIFaddbegin \DIFadd{For example, the tree in
...
to mimic the application of Algorithm \DIFdelbegin \DIFdel{\ref{alg:ConsistentSplitsFromRankedList}
}\DIFdelend \DIFaddbegin \DIFadd{$\texttt{ConsistentSplitsFromRankedList}$ (Fig. \ref{alg:ConsistentSplitsFromRankedList})
}\DIFaddend to the full (unpruned) taxonomic tree. This would be equivalent to
...
\DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:Two-approaches-to-unpruning} illustrates the two
...
N (as in the tree shown in Fig\DIFaddbegin \DIFadd{. }\DIFaddend \ref{fig:Two-approaches-to-unpruning}c),
because that is the least inclusive unbroken ancestor of R. The tree
illustrated in Fig\DIFaddbegin \DIFadd{. }\DIFaddend \ref{fig:Two-approaches-to-unpruning}d shows the
...
\DIFdelbeginFL %DIFDELCMD < \subfloat[Taxonomy]{\begin{centering}
%DIFDELCMD < \begin{minipage}[c][1\totalheight][t]{0.45\columnwidth}%
%DIFDELCMD < \begin{center}
%DIFDELCMD < \includegraphics[scale=0.38]{example3__cleaned_ott-labels\lyxdot tre}
%DIFDELCMD < \par\end{center}%
%DIFDELCMD < \end{minipage}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < }\hfill{}\subfloat[Grafted solution]{\begin{centering}
%DIFDELCMD < \begin{minipage}[c][1\totalheight][t]{0.45\columnwidth}%
%DIFDELCMD < \begin{center}
%DIFDELCMD < \includegraphics[scale=0.38]{example3__grafted_solution-labels\lyxdot tre}
%DIFDELCMD < \par\end{center}%
%DIFDELCMD < \end{minipage}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < }\hfill{}
...
%DIFDELCMD < \subfloat[Unpruned tree - broken taxa removed]{\begin{centering}
%DIFDELCMD < \includegraphics[scale=0.38]{example3__unprune-old-labels\lyxdot tre}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < }\hfill{}\subfloat[Unpruned tree with MRCA-attachment]{\begin{centering}
%DIFDELCMD < \includegraphics[scale=0.38]{example3__unprune-new-labels2\lyxdot tre}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.5]{Fig11}
\DIFaddendFL \par\end{centering}
\caption{\DIFaddbeginFL \label{fig:unprune}\DIFaddFL{Unpruned tree with internal taxa. Taxa unsampled in phylogenetic
...
%DIFDELCMD < \begin{centering}
%DIFDELCMD < \includegraphics[scale=0.5]{Figures__labelled_supertree__labelled_supertree_only_taxon_names\lyxdot tre}
%DIFDELCMD < \par\end{centering}
%DIFDELCMD < \caption{%%%
\DIFdelFL{Unpruned tree with internal taxa. Nodes E2, F2, and D have been re-added
to the tree. The nodes E and F are no longer monotypic.}%DIFDELCMD < }
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Input tree $T_{1}$ named ``tree1'']{%
%DIFDELCMD < \begin{minipage}[c][1\totalheight][t]{0.25\columnwidth}%
%DIFDELCMD < \includegraphics[scale=0.5]{annotation_example__input1\lyxdot tre}%
%DIFDELCMD < \end{minipage}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Summary Tree $\protect\summaryTree$]{%
%DIFDELCMD < \begin{minipage}[c][1\totalheight][t]{0.35\columnwidth}%
%DIFDELCMD < \includegraphics[scale=0.5]{annotation_example__summary\lyxdot tre}%
%DIFDELCMD < \end{minipage}
%DIFDELCMD < 

%DIFDELCMD < }\hfill{}\subfloat[Induced Tree $\protect\summaryTree(1)$]{%
%DIFDELCMD < \begin{minipage}[c][1\totalheight][t]{0.35\columnwidth}%
%DIFDELCMD < \includegraphics[scale=0.5]{annotation_example__induced1_color}%
%DIFDELCMD < \end{minipage}
%DIFDELCMD < 
...
%DIFDELCMD < \hfill{}\subfloat[JSON annotations relating edges of (c) to edges of (a)]{%
%DIFDELCMD < \shadowbox{\begin{minipage}[t]{1.0\columnwidth}%
%DIFDELCMD < \texttt{\{}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~\textquotedbl{}nodes\textquotedbl{}: \{}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~~~\textquotedbl{}mrcaott1ott4\textquotedbl{}: \{ \textquotedbl{}}\texttt{\textcolor{blue}{partial\_path\_of}}\texttt{\textquotedbl{}: \{
%DIFDELCMD < \textquotedbl{}tree1\textquotedbl{}: \textquotedbl{}node15\textquotedbl{},
%DIFDELCMD < ... \} \},}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~~~\textquotedbl{}ott10\textquotedbl{}:~~~~~~~~~\{
%DIFDELCMD < \textquotedbl{}}\texttt{\textcolor{blue}{partial\_path\_of}}\texttt{\textquotedbl{}: \{
%DIFDELCMD < \textquotedbl{}tree1\textquotedbl{}: \textquotedbl{}node15\textquotedbl{},
%DIFDELCMD < ... \} \},}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~~~\textquotedbl{}mrcaott5ott7\textquotedbl{}: \{ \textquotedbl{}\textcolor{mylime}{supported\_by}\textquotedbl{}:~~~
%DIFDELCMD < \{ \textquotedbl{}tree1\textquotedbl{}: \textquotedbl{}node16\textquotedbl{},
%DIFDELCMD < ... \} \},}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~~~\textquotedbl{}ott1\textquotedbl{}:~~~~~~~~~~\{
%DIFDELCMD < \textquotedbl{}}\texttt{\textcolor{orange}{terminal}}\texttt{\textquotedbl{}: ~~~~~~~\{
%DIFDELCMD < \textquotedbl{}tree1\textquotedbl{}: \textquotedbl{}ott1\textquotedbl{},
%DIFDELCMD < ... ~~\} \},}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~~~\textquotedbl{}ott2\textquotedbl{}:~~~~~~~~~~\{
%DIFDELCMD < \textquotedbl{}}\texttt{\textcolor{orange}{terminal}}\texttt{\textquotedbl{}: ~~~~~~~\{
%DIFDELCMD < \textquotedbl{}tree1\textquotedbl{}: \textquotedbl{}ott2\textquotedbl{},
%DIFDELCMD < ... ~~\} \},}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~~~\textquotedbl{}ott5\textquotedbl{}:~~~~~~~~~~\{
%DIFDELCMD < \textquotedbl{}}\texttt{\textcolor{orange}{terminal}}\texttt{\textquotedbl{}: ~~~~~~~\{
%DIFDELCMD < \textquotedbl{}tree1\textquotedbl{}: \textquotedbl{}ott5\textquotedbl{},
%DIFDELCMD < ... ~~\} \},}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~~~\textquotedbl{}ott11\textquotedbl{}:~~~~~~~~~\{
%DIFDELCMD < \textquotedbl{}}\texttt{\textcolor{orange}{terminal}}\texttt{\textquotedbl{}: ~~~~~~~\{
%DIFDELCMD < \textquotedbl{}tree1\textquotedbl{}: \textquotedbl{}ott5\textquotedbl{},
%DIFDELCMD < ... ~~\} \},}
%DIFDELCMD < 

%DIFDELCMD < \texttt{~~~~\textquotedbl{}ott7\textquotedbl{}:~~~~~~~~~~\{
%DIFDELCMD < \textquotedbl{}}\texttt{\textcolor{orange}{terminal}}\texttt{\textquotedbl{}: ~~~~~~~\{
%DIFDELCMD < \textquotedbl{}tree1\textquotedbl{}: \textquotedbl{}ott7\textquotedbl{},
%DIFDELCMD < ... ~~\} \}}
%DIFDELCMD < 
...
tree $\protect\summaryTree$ \DIFaddbeginFL \DIFaddFL{in panel }\DIFaddendFL (b) to edges in the input tree $T_{1}$ \DIFaddbeginFL \DIFaddFL{named ``tree1'' in panel
}\DIFaddendFL (a). Only edges of $\protect\summaryTree$ that are present in the
induced tree $\protect\summaryTree(1)$ \DIFaddbeginFL \DIFaddFL{in panel (c) }\DIFaddendFL are represented by JSON annotations
\DIFaddbeginFL \DIFaddFL{in panel }\DIFaddendFL (d). Taxon names are here suppressed in favor of OTT IDs, and edges
...
\DIFaddbegin \subsection{\DIFadd{Checking reliability of the method}}
\DIFadd{To test the reliability of our methods, we divided the \otlprop{} summary
...
\DIFaddbegin \DIFadd{Our supertree pipeline makes it possible to summarize and merge
...
\DIFaddend The migration of summary tree construction from treemachine (used for
...
the \DIFdelbegin \DIFdel{decreased
the }\DIFdelend computational time required to construct a supertree from several
...
Stephen Smith, and Joseph Brown for discussions and feedback.
\DIFdelbegin \DIFdel{Thanks
...
%DIFDELCMD < %%%
\section{\DIFdel{The maximum sum displayed groups' weighted scores criterion}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{sec:The-maximum-sum}%%%
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\section{\DIFdel{Description of the decomposition algorithm of otc-uncontested-decompose}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{sec:Decompose-algorithm}%%%
%DIFDELCMD < 

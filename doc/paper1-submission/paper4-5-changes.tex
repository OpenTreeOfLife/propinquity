...
%DIF DEL paper4.tex   Thu Oct 13 13:46:43 2016
%DIF ADD paper5.tex   Wed Jan 25 12:49:55 2017
...
that are displayed by the summary tree. Figure \DIFdelbegin \DIFdel{(\ref{fig:pairwisecompat}
)
}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:pairwisecompat}
...
shows an exampleof how }\DIFdelend \DIFaddbegin \DIFadd{For example, }\DIFaddend the trees in \DIFdelbegin \DIFdel{figure \ref{fig:cleaned-phylo-example} would be exemplified }\DIFdelend \DIFaddbegin \DIFadd{Figure
\ref{fig:cleaned-phylo-example} would exemplified as shown in Figure
...
\DIFaddbeginFL \DIFaddFL{Similarly, the trivial problems that list E1 and F1 as the only daughters
of taxa E and F have been omitted.}\DIFaddendFL }
...
by propinquity)\DIFaddbegin \DIFadd{; see figure \ref{fig:grafted-solution}}\DIFaddend . Recall that each non-root uncontested taxon used
...
will be placed}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{The nodes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{In a real analysis, the tips shown as }\DIFaddendFL E and F \DIFdelbeginFL \DIFdelFL{are monotypic }\DIFdelendFL here
\DIFaddbeginFL \DIFaddFL{would be monotypic taxa (with children E1 and F1}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{but will end
up being polytypic after unpruning is performed}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{respectively)
from the trivial subproblems that were omitted from from figure \ref{fig:subproblems-example}}\DIFaddendFL .}
...
Figure \ref{fig:grafted-solution} is unpruned to produce the tree in
Figure \ref{fig:unprune}.
}\DIFaddend 
...
\par\end{centering}
\caption{\label{fig:unprune}\DIFaddFL{Unpruned tree with internal taxa. Taxa unsampled in phylogenetic
...
Additionally, the new propinquity software pipeline has \DIFdelbegin \DIFdel{decreased the }\DIFdelend decreased

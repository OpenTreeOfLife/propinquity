...
\author{Benjamin D. Redelings$^{1,2}$, Mark T. Holder$^{1,3}$}
\maketitle
\begin{abstract}
We present a new supertree method and software implementation that
enables rapid estimation of a summary tree on the scale of millions
of leaves. This software pipeline is called \emph{``propinquity.''}
It uses the dependency-tracking tool \emph{make} to direct the series
of operations required to construct the tree. It relies heavily on
\emph{``otcetera''} - a set of C++ tools to performs most of the
steps of the pipeline. All of the components are free software and
are available on GitHub. This pipeline is currently used by the Open
Tree of Life project to produce all of the versions of project's ``synthetic
tree'' starting at version 5. The propinquity-based pipeline improves
on previous supertree construction methods because it produces no
unsupported branches, and it avoids unnecessary polytomies. In addition
to producing a tree, the pipeline writes an annotations file that
reports which grouping in the input trees support each of the groups
in the supertree.
\end{abstract}
...
\begin{figure}
\subfloat[Input tree 1]{\includegraphics[width=0.45\textwidth]{Figures/toy_ambig/abcd}

}\hfill{}\subfloat[Input tree 2]{\includegraphics[width=0.45\textwidth]{Figures/toy_ambig/bcde}

}

\subfloat[Summary tree]{\includegraphics[width=0.45\textwidth]{Figures/toy_ambig/soln1}

}\hfill{}\subfloat[Summary tree]{\includegraphics[width=0.45\textwidth]{Figures/toy_ambig/soln2}
...
\begin{figure}
\subfloat[Input tree 1]{\includegraphics[scale=0.6]{Figures/toy_pairwise_compat/abc}

}\hfill{}\subfloat[Input tree 2]{\includegraphics[scale=0.6]{Figures/toy_pairwise_compat/acd}

}\hfill{}\subfloat[Input tree 3]{\includegraphics[scale=0.6]{Figures/toy_pairwise_compat/bcd}

}

\subfloat[Summary tree]{\includegraphics[scale=0.6]{Figures/toy_pairwise_compat/s12}

}\hfill{}\subfloat[Summary tree]{\includegraphics[scale=0.6]{Figures/toy_pairwise_compat/s13}

}\hfill{}\subfloat[Summary tree]{\includegraphics[scale=0.6]{Figures/toy_pairwise_compat/s23}
...
\section{Description of the propinquity pipeline}
...
\begin{figure}
\begin{centering}
\includegraphics[height=0.75\paperheight,bb = 0 0 200 100, draft, type=eps]{paper-pipeline-tools.png}
\par\end{centering}
...
in at least one phylogenetic input. In version 5.0 of the Open Tree
of Life synthetic tree, this version of the taxonomic tree contained
only 41,226 leaves (while the flag-cleaned version of OTT had 2,424,255
leaves).
\begin{figure}
\subfloat[Tree 1]{\includegraphics[scale=0.4]{Figures/cleaned_phylo/ex_2@tree1\lyxdot tre}

}\hfill{}\subfloat[Tree 2]{\includegraphics[scale=0.4]{Figures/cleaned_phylo/ex_2@tree2\lyxdot tre}

}\hfill{}\subfloat[Taxonomy Tree]{\includegraphics[scale=0.4]{Figures/cleaned_ott/cleaned_ott\lyxdot tre}
...
\begin{figure}
\subfloat[Tree 1]{\includegraphics[scale=0.4]{Figures/exemplified_phylo/ex_2@tree1\lyxdot tre}

}\hfill{}\subfloat[Tree 2]{\includegraphics[scale=0.4]{Figures/exemplified_phylo/ex_2@tree2\lyxdot tre}

}\hfill{}\subfloat[Pruned taxonomy tree $\protect\prunedTaxonomy$]{\includegraphics[scale=0.4]{Figures/exemplified_phylo_taxonomy_uncontested\lyxdot tre}
...
tree; 2,838 subproblems were trivial in this way. The subproblem with
the largest number of tips contained 953 tips. The largest subproblem,
in terms of the number of input trees (including the taxonomic tree)
that were relevant, had 18 trees. Without decomposition, the supertree
problem would have had 512 input trees and 41,226 leaves.

\begin{figure}
  \subfloat[Subproblem \emph{ABCD}]{\includegraphics[width=0.3\textwidth]{Figures/subproblems/14/1\lyxdot tre}%
    \hfill{}\includegraphics[width=0.3\textwidth]{Figures/subproblems/14/2\lyxdot tre}%
    \hfill{}\includegraphics[width=0.3\textwidth]{Figures/subproblems/14/3\lyxdot tre}

}

  \subfloat[Subproblem \emph{root}]{\includegraphics[width=0.3\textwidth]{Figures/subproblems/17/1\lyxdot tre}%
    \hfill{}\includegraphics[width=0.3\textwidth]{Figures/subproblems/17/2\lyxdot tre}%
    \hfill{}\includegraphics[width=0.3\textwidth]{Figures/subproblems/17/3\lyxdot tre}
...
\end{figure}
\begin{figure}
\subfloat[]{\includegraphics[scale=0.6]{Figures/uncontested_worsens_score/ab1c}

}\hfill{}\subfloat[]{\includegraphics[scale=0.6]{Figures/uncontested_worsens_score/acb2}

}\hfill{}\subfloat[]{\includegraphics[scale=0.6]{Figures/uncontested_worsens_score/tax}

}

\hfill{}\subfloat[]{\includegraphics[scale=0.6]{Figures/uncontested_worsens_score/opt}

}\hfill{}\subfloat[]{\includegraphics[scale=0.6]{Figures/uncontested_worsens_score/returned}
...
\begin{figure}
\hfill{}\subfloat[Solution to subproblem \emph{ABCD}]{\includegraphics[scale=0.45]{Figures/subproblem_solutions/ott14\lyxdot tre}

}\hfill{}\subfloat[Solution to subproblem \emph{root}]{\includegraphics[scale=0.45]{Figures/subproblem_solutions/ott17\lyxdot tre}
...
\begin{figure}
\begin{centering}
\includegraphics[scale=0.5]{Figures/grafted_solution/grafted_solution_ottnames\lyxdot tre}
\par\end{centering}
...
\begin{center}
\includegraphics[scale=0.4]{example3/cleaned_ott-labels\lyxdot tre}
\par\end{center}%
\end{minipage}
\par\end{centering}
...
\begin{minipage}[c][1\totalheight][t]{0.45\columnwidth}%
\begin{center}
\includegraphics[scale=0.4]{example3/grafted_solution-labels\lyxdot tre}
\par\end{center}%
\end{minipage}
\par\end{centering}
}\hfill{}
...
\includegraphics[scale=0.4]{example3/unprune-old-labels\lyxdot tre}
\par\end{centering}
}\hfill{}\subfloat[Unpruned tree with MRCA-attachment]{\begin{centering}
\includegraphics[scale=0.4]{example3/unprune-new-labels2\lyxdot tre}
\par\end{centering}
}
\par\end{centering}
...
\begin{figure}
\begin{centering}
\includegraphics[scale=0.5]{Figures/labelled_supertree/labelled_supertree_only_taxon_names\lyxdot tre}
\par\end{centering}
...
\includegraphics[scale=0.5]{annotation_example/input1\lyxdot tre}%
\end{minipage}

}\hfill{}\subfloat[Summary Tree $\protect\summaryTree$]{%
\begin{minipage}[c][1\totalheight][t]{0.35\columnwidth}%
\includegraphics[scale=0.5]{annotation_example/summary\lyxdot tre}%
\end{minipage}

}\hfill{}\subfloat[Induced Tree $\protect\summaryTree(1)$]{%
\begin{minipage}[c][1\totalheight][t]{0.35\columnwidth}%
\includegraphics[scale=0.5]{annotation_example/induced1_color}%
\end{minipage}
...
\shadowbox{\begin{minipage}[t]{0.8\columnwidth}%
\texttt{\{}
...
and a node provenance object categorizes the relationship between
...
Conceptually, this annotation operation in equivalent to considering
...
that contains $x$ are stored in a ``conflicts\_with'' property
...
\begin{itemize}
\item if $x$ is a terminal path in the induced tree then node $j$ will
be listed in the ``terminal'' property;
\item if $x$ is along an internal path that contains some nodes with out-degree
equal to 1, then node $j$ will be listed in the ``partial\_path\_of''
property; and
\item if $x$ is along an internal path without any node of out-degree 1,
then node $j$ will be listed in the ``supported\_by'' property,
...
``resolved\_by'' property of $x$. However, because of our goal
...
has also decreased substantially. We ran propinquity on the same input
trees used to construct version Open Tree version 4. Unlike the version
4 tree, the tree from propinquity contained no unsupported nodes or
unnecessary polytomies. The number of displayed input splits increased
from 35,518 (for version 4) to 39,639; an 11.2\% increase. The number
of input splits that do not conflict with the summary tree, but are
not incorporated dropped from 2718 to 0. The number of input splits
that conflict with the phylogeny dropped from 2760 to 1357, a decrease
of 1357, or 49.2\%. %
\begin{comment}
ARE THESE numbers for the same set of inputs? or v4 to v5 (the latter
had the barnacles tree collection, and perhaps a few other tree differences?
\end{comment}
...
\section{\label{sec:The-maximum-sum}The maximum sum displayed groups' weighted
scores criterion}
...
\section{\label{sec:Decompose-algorithm}Description of the decomposition
algorithm of otc-uncontested-decompose}
